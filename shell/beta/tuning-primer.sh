...
check_for_socket () {
	if [ -z "$socket" ] ; then
		...
		if [ -f /etc/mysql/my.cnf ] ; then
			cnf_socket=$(grep ^socket /etc/mysql/my.cnf | awk -F \= '{ print $2 }' | head -1)
		fi
...
## -- Test for running mysql -- ##
	...
	if [ "$is_up" = "mysqld is alive" ] ; then
		...
		printf "\n"
		cecho "Using login values from /etc/mysql/my.cnf" 
...
## -- create a /etc/mysql/my.cnf and exit when all else fails -- ##
	...
	case $REPLY in 
		...
		*)
		...
		cecho "Or, set correct values for  'user=' and 'password=' in /etc/mysql/my.cnf"
		;;
	esac
	cecho " "
	read -p "Would you like me to create a /etc/mysql/my.cnf file for you? [y/N] : " REPLY
        case $REPLY in
	        yes | y | Y | YES)
		answer2='yes'
		if [ ! -f /etc/mysql/my.cnf ] ; then
			umask 077
			printf "[client]\nuser=$user\npassword=$pass\nsocket=$socket" > /etc/mysql/my.cnf
			if [ "$answer1" != 'yes' ] ; then
				exit 1
			else
				final_login_attempt
			...
			printf "\n"
			cecho "/etc/mysql/my.cnf already exists!" boldred
			printf "\n"
			read -p "Replace ? [y/N] : " REPLY
			if [ "$REPLY" = 'y' ] || [ "$REPLY" = 'Y' ] ; then 
			printf "[client]\nuser=$user\npassword=$pass\socket=$socket" > /etc/mysql/my.cnf
				if [ "$answer1" != 'yes' ] ; then
					exit 1
				else
					final_login_attempt
				...
				cecho "Please set the 'user=' and 'password=' and 'socket=' values in /etc/mysql/my.cnf"
				exit 1
			fi
		fi
		;;
...
## -- populate the .my.cnf file using values harvested from Webmin -- ##
	...
	if [ -f /etc/webmin/mysql/config ] ; then
		...
		if [  $user ] && [ $pass ] && [ ! -f /etc/mysql/my.cnf  ] ; then
